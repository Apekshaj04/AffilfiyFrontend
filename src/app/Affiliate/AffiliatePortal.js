...
export default function AffiliatePortal() {
  ...
  const fetchRequestNotifications = async (affiliateAddress) => {
    try {
        const response = await fetch(`http://54.147.55.29:8080/api/request/getRequest/${affiliateAddress}`);
        ...
        if (data.length > 0 && !requestNotification) {
            ...
            const companyResponse = await fetch(`http://54.147.55.29:8080/api/company/getCompanyByWallet/${data[0].companyAddress}`);
            ...
            const productResponse = await fetch(`http://54.147.55.29:8080/api/product/${data[0].product}`);
  ...
  const deleteNotification = async (id) => {
    try {
      await fetch(`http://54.147.55.29:8080/api/request/${id}`, {
        method: 'DELETE'
      });
  ...
  const handleAcceptRequest = async () => {
    ...
    try {
        ...
        const notify = await fetch(
            `http://54.147.55.29:8080/api/request/getRequest/${walletAddress}`
        );
        ...
        const response = await fetch(
            `http://54.147.55.29:8080/api/affiliate/affiliateProduct/${productID}`, 
            {
                method: 'POST',
                headers: {
