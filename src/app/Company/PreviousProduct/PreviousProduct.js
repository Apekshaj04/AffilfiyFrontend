...
export default function PreviousProduct() {
  ...
  useEffect(() => {
    ...
    // Fetch products
    fetch(`http://54.147.55.29:8080/api/company/getProductsByCompany/${walletAddress}`)
      .then((res) => res.json())
      .then((data) => {
  ...
  const fetchTransactions = async (companyAddress) => {
    ...
    try {
      const response = await fetch(`http://54.147.55.29:8080/api/transact/company/${companyAddress}`);
