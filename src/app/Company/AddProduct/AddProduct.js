...
export default function AddProduct() {
  ...
  useEffect(() => {
    const fetchCompany = async () => {
      ...
      if (walletAddress) {
        try {
          const response = await fetch(`http://100.31.0.34:8080/api/company/getCompanyByWallet/${walletAddress}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        const response = await fetch(`http://100.31.0.34:8080/api/company/addProduct/${walletAddress}`, {
            method: 'POST',
            headers: {
